...
// Connects to PocketBase as superadmin and:
//   1. Adds role + mustChangePassword fields to _pb_users_auth_
//   2. Creates all 6 collections with fields and access rules
//   3. Creates the initial admin@lattice.dev user account
//
// Run once against a fresh PocketBase instance AFTER creating your superadmin
// account in the PocketBase admin UI (http://<host>:8090/_/).
//
// Usage:
//   npx ts-node --project scripts/tsconfig.json scripts/seed.ts \
//     --url http://<host>:8090 \
//     --email <pb-superadmin-email> \
//     --password <pb-superadmin-password>
...
async function upsertCollection(data: object): Promise<void> {
...
async function setupUsersCollection() {
  ...
  const existingFields: string[] = (col.schema ?? (col as any).fields ?? []).map((f: any) => f.name);
  ...
  if (!existingFields.includes("role")) {
    newFields.push({
      ...
      type: "select",
      required: true,
      options: { maxSelect: 1, values: ["admin", "manager", "office", "shopfloor"] },
    });
  }
  ...
  if (newFields.length === 0) {
    console.log("  ↩ Fields already present, skipping");
    return;
  }

  const currentFields = col.schema ?? (col as any).fields ?? [];
  await pb.collections.update("_pb_users_auth_", {
    schema: [...currentFields, ...newFields],
  });
  console.log(`  ✓ Added: ${newFields.map((f: any) => f.name).join(", ")}`);
}
