      ...
      <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "1.25rem", flex: 1 }}>
          ...
          {tab === "dashboard" && (() => {
            ...
            const myTasks = currentUser.role === ROLES.WORKER
            ...
            const dueSoonOrders = suppliers.flatMap(s =>
              (s.orders||[]).filter(o => {
                ...
                return !o.arrived && due >= now && due <= in7;
            ...
            const StatCard = ({ icon, value, label, color, onClick, urgent }) => (
              <div onClick={onClick} style={{ background: "#0f0f1e", border: `1px solid ${urgent && value > 0 ? color + "55" : "#1e1e35"}`, borderRadius: "10px", padding: "1rem 1.25rem", cursor: onClick ? "pointer" : "default", display: "flex", alignItems: "center", gap: "1rem", transition: "border-color 0.2s" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0.75rem", background: "#0f0f1e", borderRadius: "8px", marginBottom: "4px", border: "1px solid #1a1a2e" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.68rem", color: "#555", marginTop: "1px" }}>
                      ...
                      {assignee && currentUser.role !== ROLES.WORKER && <span style={{ marginLeft: "0.4rem" }}>· {assignee.name}</span>}
                    </div>
                  </div>
                  <div style={{ fontSize: "0.72rem", color: overdue ? "#fc8181" : "#f6c90e", whiteSpace: "nowrap", flexShrink: 0 }}>
                    {overdue ? `${daysBetween(task.endDate, now)}d overdue` : `Due ${fmt(task.endDate)}`}
                  </div>
                  <select value={task.status} onChange={e => { setTasks(p => p.map(x => x.id === task.id ? { ...x, status: e.target.value } : x)); }} style={{ padding: "2px 4px", background: `${statusColor[task.status]}18`, border: `1px solid ${statusColor[task.status]}55`, borderRadius: "4px", color: statusColor[task.status], fontSize: "0.68rem", cursor: "pointer", flexShrink: 0 }}>
            ...
            return (
              <div>
                ...
                <div style={{ marginBottom: "1.5rem" }}>
                  ...
                  <p style={{ color: "#555", fontSize: "0.82rem", marginTop: "4px" }}>{new Date().toLocaleDateString("en-AU", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
                </div>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
                  ...
                  <div style={{ display: "grid", gap: "1.25rem", alignContent: "start" }}>
                    ...
                    {(overdueOrders.length > 0 || dueSoonOrders.length > 0) && (
                      <div>
                        ...
                        {[...overdueOrders.map(o => ({ ...o, isLate: true })), ...dueSoonOrders].slice(0, 5).map(o => (
                          <div key={o.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 0.75rem", background: "#0f0f1e", borderRadius: "8px", marginBottom: "4px", border: "1px solid #1a1a2e" }}>
                            ...
                            <div style={{ fontSize: "0.72rem", color: o.isLate ? "#fc8181" : "#f6c90e", whiteSpace: "nowrap", flexShrink: 0 }}>
                  ...
                  <div style={{ display: "grid", gap: "1.25rem", alignContent: "start" }}>
                              ...
                              <div key={proj.id} style={{ padding: "0.75rem 1rem", borderTop: i > 0 ? "1px solid #141428" : "none" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                                    <div style={{ width: "8px", height: "8px", borderRadius: "2px", background: proj.color, flexShrink: 0 }} />
                            ...
                            return (
                              <div key={u.id} style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.6rem 1rem", borderTop: i > 0 ? "1px solid #141428" : "none" }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div style={{ fontSize: "0.65rem", color: "#444" }}>{total} open task{total !== 1 ? "s" : ""}</div>
                                </div>
                                {late > 0 && <span style={{ fontSize: "0.68rem", color: "#fc8181", background: "#fc818118", padding: "1px 6px", borderRadius: "3px", flexShrink: 0 }}>{late} late</span>}
                                {total === 0 && <span style={{ fontSize: "0.68rem", color: "#48bb78", flexShrink: 0 }}>✓ Clear</span>}
                              </div>
                            );
                          })}
                        </div>
                    ...
                    {/* My active tasks (worker view) */}
                    {currentUser.role === ROLES.WORKER && inProgressTasks.length > 0 && (
                      <div>
